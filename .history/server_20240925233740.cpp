...
json getDataELF(const string& pathfile) {
    json elfContent;
    ...
    int fd = open(pathfile.c_str(), O_RDONLY, 0);
    if (fd < 0) {
        cerr << "Failed to open file: " << pathfile << endl;
    ...
    if (gelf_getehdr(elf, &ehdr) == nullptr) {
        cerr << "gelf_getehdr() failed: " << elf_errmsg(-1) << endl;
        elf_end(elf);
        close(fd);
    ...
    while ((section = elf_nextscn(elf, section)) != nullptr) {
        if (gelf_getshdr(section, &shdr) != &shdr) {
            cerr << "gelf_getshdr() failed: " << elf_errmsg(-1) << endl;
            continue;
        }
...
int main() {
    ...
    CROW_ROUTE(app, "/upload").methods(crow::HTTPMethod::POST)([](const crow::request& req) {
        ...
        ofstream ofs("/tmp/file.elf", ios::binary);
        ofs.write(fileContent.c_str(), fileContent.size());
        ofs.close();

        json result = getDataELF("/tmp/file.elf");
