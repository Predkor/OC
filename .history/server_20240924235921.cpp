...
json getProcessInfo(const string& pid) {
    ...
    process_info["user"] = get_username(uid);

    // Чтение файла /proc/[pid]/stat для получения PR, NI, времени процессора
    ...
    if (stat_file) {
        ...
        stat_file.ignore(256, ')');  // Пропуск имени команды
        for (int i = 0; i < 11; ++i) stat_file.ignore(256, ' ');  // Пропуск ненужных полей
        stat_file >> utime >> stime;  // Время процессора
        for (int i = 0; i < 6; ++i) stat_file.ignore(256, ' ');  // Пропуск ненужных полей
        stat_file >> vsize >> rss;

        process_info["pr"] = pr;
        process_info["ni"] = ni;
        process_info["virt"] = vsize / 1024;  // Преобразование в КБ
        process_info["res"] = rss * sysconf(_SC_PAGESIZE) / 1024;  // Преобразование в КБ
        process_info["shr"] = "N/A";  // SHR можно получить через доп. работу с `/proc/[PID]/statm`, но пока оставим так
        process_info["%cpu"] = "N/A";  // %CPU требует времени системы, оставим для расчета позже
        process_info["%mem"] = "N/A";  // %MEM также требует информации о памяти системы
    }

    // Чтение файла /proc/[pid]/cmdline для команды
