...
json getProcessInfo(const string& pid, unsigned long totalMemory, unsigned long totalTimeSystem) {
    json processInfo;
    processInfo["pid"] = pid;
    ...
    if (stat_file) {
        ...
        if (totalProcessTime > 0) {
            processInfo["%cpu"] = (totalProcessTime * 100 / totalTimeSystem);
        } else {
            processInfo["%cpu"] = "N/A";
        }

        if (totalMemory > 0) {
            processInfo["%mem"] = rss * sysconf(_SC_PAGESIZE) * 100.0 / totalMemory * 1024;
        }
...
vector<json> getProcessList() {
    ...
    unsigned long totalTimeSystem = getTotalSystemTime();

    while ((entry = readdir(dir)) != NULL) {
       if (entry->d_type == DT_DIR) {
            string name(entry->d_name);
            if (all_of(name.begin(), name.end(), ::isdigit)) {
                processes.push_back(getProcessInfo(name, totalMemory, totalTimeSystem));
            }
       }
    }
