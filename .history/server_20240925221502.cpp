...
vector<json> killProcess(const string& pid) {
    vector<json> result;

    pid_t p = atoi(pid.c_str());
    if (p <= 0) {
        result.push_back({{"status", "error"}, {"message", "Invalid PID"}});
        return result;
    }
    ...
    if (status == 0) {
        result.push_back({{"status", "success"}, {"message", "Process killed"}});
    } else {
        result.push_back({{"status", "error"}, {"message", "Failed to kill"}});
    }
